...
class VirtualMachine:
    ...
    def __init__(self, name):
        ...
        #self.template_15 = '{}.15.json'.format(name)
        self.template_15 = '{}_stable.json'.format(name)
    ...
    def removevm(self, build=None):
        ...
        if 'stable' in self.name:
            ...
            group = paths.vm_group
        try:
            shutil.rmtree(os.path.join(mf, group, self.name))
    ...
    def buildvm(self, build=None):
        """Build and export the virtual machine. Add '-debug' to a
        subprocess.call to turn debuging on"""
        if build == 'stable':
            template = self.template_15
        else:
            template = self.template
        templ = os.path.join(self.dir, template)
        assert os.path.exists(templ), "%s not found" % template
        ...
        subprocess.call([packer_main, 'build', '-force',
                        '-var', 'headless=true', template])
        os.chdir(curdir)
        if build == 'stable':
             return os.path.join(self.dir, paths.packer_export,
                                self.name + '_stable.ova')
        else:
            return os.path.join(self.dir, paths.packer_export,
                                self.name + '.ova')

    def _groupvm(self, build=None):
        #if build == 'stable':
        if 'stable' in self.name:
            group = '/' + paths.vm_group_stable
        else:
...
def build_vm(vmname, build=None):
    ...
    try:
        ...
        v_machine.removevm()
    if build == 'stable':
        return v_machine.buildvm('stable')
    else:
...
class Builder(VMHandler):
    ...
    """
    def _build_pool(self, procs, lst, build=None):
        pool = multiprocessing.Pool(processes=procs)
        for vm in lst:
            if build == 'stable':
                pool.apply_async(build_vm, args=(vm,'stable'), callback=self._callback)
    ...
    @staticmethod
    def _upload_dir(build=None):
        """Create the directory using current date."""
        if build == 'stable':
            upldir = os.path.join(paths.upload, time.strftime('%d-%m-%Y' + '_stable'))
        else:
            ...
            try:
                ...
                if ignore_missing:
                    ...
                    if (imgexc.errno == errno.ENOENT and
                        ...
                        print("{} is missing. Skipping...".format(image), file=stderr)
                    else:
                        raise
                else:
                    raise
...
class Interface:
    ...
    def __init__(self):
        ...
        parser_build.add_argument('-s', '--stable',
                                  action='store_true',
                                  help='1.5 stable'
                                  )
